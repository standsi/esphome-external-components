...
riscv_blink_ns = cg.esphome_ns.namespace("riscv_blink")
RISCVBlinkComponent = riscv_blink_ns.class_("RISCVBlinkComponent", cg.Component)
...
InitState = riscv_blink_ns.enum("InitState", is_class=True)
...
def _validate_variant(config):
    ...
    if variant not in (VARIANT_ESP32S2, VARIANT_ESP32S3):
        raise cv.Invalid("riscv_blink only supports ESP32-S2 and ESP32-S3 with ESP-IDF")
    return config
...
CONFIG_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(RISCVBlinkComponent),
